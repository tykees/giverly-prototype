  ...
  return (
    ...
    <div className='container mx-auto ml-5'>
        <div className='flex flex-col gap-40 justify-center items-start md:justify-around md:flex-row mx-auto space-y-0 md:space-y-0 pb-40 pt-40'>
            <div>
                <Image className='w-[350px] md:w-[200px] shadow-xl' src={Flowlogo} alt='userimg'></Image>
                <p className='border-b border-slate-500 w-[200px] mt-5'></p>
                <p className='text-2xl'>Maisha Flow</p> 
                <p className='flex justify-start gap-1 items-center mt-3 text-slate-500 s'> <IoSettingsOutline /> Setting</p>
               <Link href={'/charity/email'}><p className='flex cursor-pointer justify-start gap-1 items-center mt-2 text-slate-500 '> <FaRegMoneyBill1 /> All Donor</p></Link> 
               <Link href={'/charity/charity-record-donation'}><p className='flex cursor-pointer justify-start gap-1 items-center mt-2 text-slate-500 '> <FaRegMoneyBill1 /> All Expenses</p></Link> 
                <p className='mt-10 font-bold'>Aims</p>
                <ul className='text-sm text-slate-500'>
                    <li>Care for the Young</li>
                    <li>Care for Women</li>
                    <li>Care for the Sick</li>
                </ul>
                
                <p className='text-center rounded-md shadow btn-bg text-slate-600 mt-10 px-3 py-2'>Edit Profile</p>

            </div>
            <div>
                <div className='flex justify-start items-center'>
                <p className='flex text-slate-500 text-sm justify-start items-center'><IoLocationOutline /> Kenya</p>
                </div>
                {/* <p className='text-slate-500 text-sm'>Software Developer</p> */}
                <p className='mt-5'>Trust Rank</p>
                <p className='flex justify-start items-center gap-2 text-xl text-slate-400'>8.6<FaStar /><FaStar /><FaStar /><FaStar /><FaStarHalfAlt /></p>
                <div className='flex justify-start gap-2 items-start'>
                    <button className='mt-10 text-center bg-slate-500 text-white px-3 py-2 rounded-lg'>Send Message</button>
                    <button className='mt-10 text-center btn-bg text-slate-600 px-3 py-2 rounded-lg' >Discount</button>
                    <button className='mt-10 text-center bg-slate-500 text-white px-3 py-2 rounded-lg'>Update Acct</button>
                </div>
                ...
                <p className='mt-5'>Email <span className='ml-10 text-slate-500'>admin@flow.com</span> </p>
            ...
            <div>
                ...
                <p className='text-sm mb-2 text-white font-bold'>Credit from Giverly: <span className='text-green-300'>$40,000</span></p>
                <p className='text-sm text-white font-bold'>Last Credit from Giverly: <span className='text-green-300'>$2,000</span></p>
                <p className='text-sm text-white mt-2 font-bold'>Date: <span className='text-green-300'>22-12-2024</span></p>
              <Link href={'/charity/charity-giverly'}> <p className='my-2 border shadow-lg text-slate-200 text-center hover:text-green-400 hover:border-green-400 rounded-md px-3 cursor-pointer'>All Transactions</p></Link> 
                </div>
